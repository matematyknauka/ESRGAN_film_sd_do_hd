...
Google Colab: Środowisko wykonawcze T4 GPU. Zalogować się tokenem do Github jeśli repozytorium jest prywatne i tego nie zrobiłeś. Przejść do katalogu %cd ścieżka do Dysku Google np. %cd /content/drive/MyDrive, sklonować to repozytorium, zainstalować pakiety !pip install moviepy opencv-python torch torchvision numpy, uruchomić !python test.py
